...
import com.pcpl.spoolman.ndt.TestSpoolmanNDT_TestOfferingHIOff;
import com.pcpl.spoolman.ndt.TestSpoolmanNDT_TestOfferingPWHTOffering;

public class NDT_TestOffHIOff extends BaseClass{
	
	@Test
	public void HIOfferingAll() throws Throwable {
...
//		HOME PAGE		
		...
		TestSpoolmanNDT_TestOfferingHIOff offering = 
				                 new TestSpoolmanNDT_TestOfferingHIOff(driver);
		
		webdriverutility.ClickableElement(driver,sidebar.getNDTHIOfferingList_Sidebar());
		webdriverutility.validation(driver,sidebar.getNDTHIOfferingList_Sidebar(),
				"HI OFFERING SIDEBAR");				
		sidebar.getNDTHIOfferingList_Sidebar().click();
		Reporter.log("HI OFFRING SIDEBAR is FUNCTIONING PROPERLY ",true);
...
//		offering.getPleaseselecttheofferingNumber_POPUPClose().click();
		...
		webdriverutility.validation(driver,offering.getSpoolNo3Dots(),"SELECTION SELECT ALL");				
		offering.getSpoolNo3Dots().click();
		Reporter.log("SPOOL NO 3 DOTS FUNCTIONING PROPERLY",true);
		
		webdriverutility.ClickableElement(driver,offering.getFunnelFilter());
		act.moveToElement(offering.getFunnelFilter());
...
//		webdriverutility.textConfirmation(driver,contractor.getFilterValue(),s,"FILTER VALUE");
		...
		String OfferingNo ="OfferingNo_11"; 
